...
func (gc *GradeCalculator) calculateNumericalGrade() int {
	...
	essay_average := computeAverage(gc.essays) // fix: was using gc.exams
...
func computeAverage(grades []Grade) int {
	if len(grades) == 0 {
		return 0 // fix: prevent divide by zero
	}
	sum := 0

	for _, g := range grades { // fix: iterate over values, not indexes
		sum += g.Grade
	}
